...
public class EntityMinion extends EntityLiving implements IEntityWithShield {
    ...
    private static final DataParameter<Integer> TYPE = EntityDataManager.createKey(EntityMinion.class, DataSerializers.VARINT);
    private static final String TAG_SHIELD = "shield";
    private static final DataParameter<Integer> SHIELD = EntityDataManager.createKey(EntityMinion.class, DataSerializers.VARINT);
    ...
    public void onLivingUpdate() {
        ...
        for(EntityGaiaPro g : getHostAround())
    ...
    private List<EntityGaiaPro> getHostAround() {
        BlockPos source = this.getPosition();
        float range = 15F;
        return world.getEntitiesWithinAABB(EntityGaiaPro.class, new AxisAlignedBB(source.getX() + 0.5 - range, source.getY() + 0.5 - range, source.getZ() + 0.5 - range, source.getX() + 0.5 + range, source.getY() + 0.5 + range, source.getZ() + 0.5 + range));
    }
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(21);
